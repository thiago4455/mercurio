...
class ClassEmpresas {
    ...
    public function EditarEmpresa($objCadastro) {
        ...
        try {
            if($objConexao->executarComandoSQL("UPDATE `Empresas` SET `cnpj` = '$cnpj', `nomeFantasia` = '$nomeFantasia', `razaoSocial` = '$razaoSocial', `telefone` = '$telefone', `email` = '$email', `responsavel` = '$nomeResponsavel', `cep` = '$cep', `rua` = '$endereco', `numero` = '$numero', `bairro` = '$bairro', `cidade` = '$cidade', `estado` = '$estado' WHERE `codEmpresa` = '$apelido';")) {
                return true;
            }
        }
        catch(Exception $err) {
    ...
    public function BuscaEmpresasCod($busca) {
        ...
        $objConexao = new ConexaoClass("localhost", "root", "root", "dbmercurio");
                ...
                $objConexao->executarComandoSQL('SET character_set_results=utf8');        
        try {
            $tableEmpresas = $objConexao->selecionarDados("SELECT * FROM Empresas WHERE (codEmpresa LIKE '%$busca%')");
